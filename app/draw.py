...
def draw_pygame():
    '''
    Função que desenha na janela,lib usada (pygame)
    '''
    init()
    scenery = Scenery()
    ...
    glLoadIdentity()                          
    gluLookAt(app.cam_x, app.cam_y, app.cam_z, app.cam_at_x, app.cam_at_y, 0, 0, -1, 1)                     # Configurando posição da câmera
    ...
    glPushMatrix()
    glTranslatef(app.index_car, 10, 1.1) 
    glScale(1.5, 1.5, 1.5)
...
def draw_glut():
    '''
    Função que desenha na janela,lib usada (glut)
    '''
    init()
    scenery = Scenery()
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)                                # Sempre antes de desenhar qualquer coisa, deve-se limpar o frame-buffer
